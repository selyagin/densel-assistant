...
const BUILD_ID = '2026-08-28.5-advance-fix';
...
function computeAdvanceStreaks(){
  // "Оплата заранее" — это самый дальний будущий период, полностью оплаченный
  // по каждому поставщику. Раньше здесь требовалась непрерывная цепочка месяцев
  // начиная со следующего сразу за текущим — из-за этого одиночный платеж
  // "с запасом" (например, оплата за ноябрь при пустых сентябре/октябре) не
  // засчитывался вообще. Теперь считаем максимум по всем будущим оплаченным
  // периодам независимо от непрерывности.
  const streaks = {};
  const cur = currentPeriod();
